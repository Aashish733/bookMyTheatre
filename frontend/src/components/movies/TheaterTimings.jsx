...
const TheaterTimings = ({ movieId }) => {
  ...
  const effectiveState = (() => {
    const fromRoute = routeState ? decodeURIComponent(routeState) : null;
    if (fromRoute && fromRoute !== "null" && fromRoute !== "undefined") {
      return fromRoute;
    }
    return location;
  })();
  ...
  const { data: showData, isLoading } = useQuery({
    queryKey: ["show", movieId, effectiveState, formattedDate],
    queryFn: async () =>
      await getShowsByMovieAndLocation(movieId, effectiveState, formattedDate),
    placeholderData: keepPreviousData,
    select: (res) => res.data,
    enabled: !!movieId && !!effectiveState,
  });

  if (!effectiveState) {
    return (
      <div className="text-center text-gray-500 py-6">
        Waiting for your location. Allow location access in the browser, then refresh.
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="text-center text-gray-500 py-6">Loading show timings...</div>
    );
  }
      ...
      <div className="space-y-8 px-4 mb-10">
        {showData?.length === 0 && (
          <div className="text-center text-gray-500">
            No shows available in {effectiveState} for the selected date.
          </div>
        )}
        {showData?.map((curr, i) => (
          <div key={i}>
                ...
                return (
                  <button
                    onClick={() => {
                      ...
                      navigate(
                        `/movies/${movieId}/${movieName}/${effectiveState}/theater/${theaterId}/show/${slot._id}/seat-layout`
                      );
                    }}
                    key={j}
