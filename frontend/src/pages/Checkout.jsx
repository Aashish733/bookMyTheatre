...
const Checkout = () => {
  ...
  const bookTicketMutation = useMutation({
    ...
    onSuccess: (data) => {
      const msg = data?.data?.message || "Booking confirmed!";
      if (data?.data?.emailSent) {
        toast.success(msg);
      } else if (data?.data?.emailError) {
        toast.error(`Booked, but email failed: ${data.data.emailError}`);
      } else {
        toast.success(msg);
      }
      socket.emit("unlock-seats", {
      ...
      navigate(
        bookingId ? `/booking/confirmation/${bookingId}` : `/profile/${user._id}/booking`,
        {
          state: {
